...
it('it exposes the API', function () {
    $api = $this->newsletter->getApi();

    expect($api)->toBe($this->mailChimpApi);
});
...
it('can get member tags', function () {
    ...
    expect($this->newsletter->getTags($email))->toBe('all-the-member-tags');
});
...
it('can add member tags', function () {
    ...
    expect(
        $this->newsletter->addTags(['tag-1', 'tag-2'], $email)
    )->toBe('the-post-response');
});
...
it('can remove member tags', function () {
    ...
    expect(
        $this->newsletter->removeTags(['tag-1', 'tag-2'], $email)
    )->toBe('the-post-response');
});
